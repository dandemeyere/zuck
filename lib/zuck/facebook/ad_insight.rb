...
module Zuck
  class AdInsight < RawFbObject
    ...
    def self.get(graph_object_id:, range:, level: :ad, fields: %w(ad_id objective impressions unique_actions cost_per_unique_action_type clicks cpc cpm ctr spend))
      resource_url = "#{rest_path}/#{graph_object_id}/insights"
      query = {
        access_token: Zuck.graph.access_token,
        level: level,
        fields: fields.join(','),
        time_increment: 1,
